...
echo CGO_ENABLED=0 GOARCH=${GOARCH} go build -o ${PRO_ROOT}/main -ldflags "${LDFLAGS}" ${PRO_ROOT}/main.go
...
#    ;;
  "build") #使用master构建测试版本
    ...
    CGO_ENABLED=0 GOARCH=${GOARCH} go build -o ${PRO_ROOT}/main -ldflags "${LDFLAGS}" ${PRO_ROOT}/main.go
    ;;
  "docker-local") #使用本地编译二进制文件打包docker
    ...
    ;&
  "docker") #使用容器编译和打包
    docker build -t hulining/golang-study-notes:$TAG_NUM $build_arg \
      --build-arg LDFLAGS="${LDFLAGS}" \
      --build-arg GOARCH=${GOARCH} \
      -f ${Dockerfile:=Dockerfile} .
    [ -n "${DockerUser}" ] && {
      echo "${DockerPass}" | docker login -u "${DockerUser}" --password-stdin
      docker push hulining/golang-study-notes:$TAG_NUM
    }
    ;;
  "clean")
    rm -f main
